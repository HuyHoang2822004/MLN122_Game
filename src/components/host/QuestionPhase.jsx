...
const CHOICE_STYLES = {
  A: { bg: 'linear-gradient(135deg,#1D4ED8,#2563EB)', border: '#2563EB', label: '▲' },
  B: { bg: 'linear-gradient(135deg,#0F766E,#14B8A6)', border: '#14B8A6', label: '◆' },
  C: { bg: 'linear-gradient(135deg,#D97706,#F59E0B)', border: '#F59E0B', label: '●' },
  D: { bg: 'linear-gradient(135deg,#DC2626,#EF4444)', border: '#EF4444', label: '■' },
};

const QuestionPhase = ({ mission, votes, votedCount, totalPlayers, timeRemaining, onForceEnd }) => {
    ...
    <div className="host-layout flex flex-col min-h-screen">
      ...
      >
        ...
        <div className="flex items-center gap-2 text-sm" style={{ color: '#475569' }}>
          <span>👥</span>
          <span>{votedCount} / {totalPlayers > 0 ? totalPlayers : '—'} đã trả lời</span>
        </div>
        <button
          onClick={onForceEnd}
            ...
            {mission.choices.map((choice, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ background: style.bg, boxShadow: `0 4px 20px ${style.border}30` }}
                >
                  <div className="px-4 py-3 flex items-center gap-3">
                    <span
                      className="text-2xl font-black flex-shrink-0"
                      style={{ fontFamily: "'Outfit', sans-serif", opacity: 0.6 }}
                    >
                      {style.label}
                    </span>
                    <span
                      className="font-semibold text-sm leading-snug text-white flex-1"
                    >
                      <span className="font-black mr-1">{choice.id}.</span> {choice.text}
                    </span>
                    {totalVotes > 0 && (
                      <span className="text-xs font-bold text-white opacity-70 ml-auto">{pct}%</span>
                    )}
                  </div>
                  {/* Vote progress bar */}
        ...
        >
          ...
          <div className="w-full max-w-xs">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-semibold tracking-wider uppercase" style={{ color: '#475569' }}>
                Đã trả lời
              </span>
              <span className="text-lg font-black" style={{ fontFamily: "'JetBrains Mono'", color: '#60A5FA' }}>
                {votedCount} <span style={{ color: '#334155', fontSize: 14 }}>/ {totalPlayers || '∞'}</span>
              </span>
            </div>
